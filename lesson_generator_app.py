"""
Somerset Lesson Generator App (Stage 7.1)
Streamlit-based GUI for managing and generating lesson plans.
"""
...
titles = sorted(week_df["Lesson Title"].dropna().unique().tolist())
selected_title = st.sidebar.selectbox("Select Lesson", titles)

# Find selected row
selected_row = week_df[week_df["Lesson Title"] == selected_title].head(1)
...
if st.sidebar.button("➕ Add New Lesson"):
    ...
    new_row["Week"] = selected_week
    new_row["Date"] = datetime.now().strftime("%B %d, %Y")
    new_row["Lesson Title"] = f"New Lesson {datetime.now().strftime('%H%M%S')}"
...
if not selected_row.empty:
    ...
    with form_cols[0]:
        for field in [
            "Lesson Title", "Date", "NV Standard Code", "NV Standard Descriptor",
...
def generate_pdf(row):
    ...
    title = str(row.get("Lesson Title", "")).strip()
...
st.dataframe(week_df[[c for c in week_df.columns if c not in ['Last Updated (PT)', 'Source Version']]], use_container_width=True)
