"""
Somerset Lesson Generator App (Stage 7.1v3)
Final stable version with week sorting fix and full Somerset PDF support.
"""
...
# -------------------------------------------------
try:
    week_values = [str(w).strip() for w in df[week_col].dropna().unique().tolist() if str(w).strip()]
    if not week_values:
        week_values = ["1"]
except Exception:
    week_values = ["1"]
...
try:
    week_values = sorted(week_values, key=lambda x: int(''.join(filter(str.isdigit, x))) if any(ch.isdigit() for ch in x) else x)
except Exception:
...
week_df = df[df[week_col].astype(str).str.strip() == str(selected_week).strip()]
